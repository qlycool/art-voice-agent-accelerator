...
class AcsDiagnostics:
    ...
    def _get_credential(self):
        ...
        try:
            ...
            if os.getenv('WEBSITE_SITE_NAME') or os.getenv('CONTAINER_APP_NAME'):
                logger.info("Using ManagedIdentityCredential for Azure-hosted environment")
        ...
        for var_name, var_value in required_vars.items():
            if not var_value:
                ...
                if var_name == 'ACS_CONNECTION_STRING':
                    ...
                    logger.info(f"✅ {var_name} is set (endpoint: {var_value.split(';')[0] if ';' in var_value else 'unknown'})")
                else:
    ...
    def test_acs_authentication(self):
        ...
        try:
            ...
            if self.acs_connection_string:
                ...
                client = CallAutomationClient(endpoint=endpoint, credential=self.credential)
            else:
                logger.error("❌ Neither ACS_CONNECTION_STRING nor ACS_ENDPOINT is available")
                return False
    ...
    def validate_phone_number_format(self):
        ...
        if not self.source_phone_number.startswith('+'):
            logger.error(f"❌ Phone number must start with '+': {self.source_phone_number}")
            return False
        
        # Remove '+' and check if remaining is digits
        number_part = self.source_phone_number[1:]
        if not number_part.isdigit():
            logger.error(f"❌ Phone number contains non-digit characters: {self.source_phone_number}")
            return False
        
        if len(number_part) < 10 or len(number_part) > 15:
            logger.error(f"❌ Phone number length invalid (should be 10-15 digits): {self.source_phone_number}")
            return False
    ...
    def check_callback_url(self):
        ...
        if 'localhost' in self.callback_url or 'devtunnels' in self.callback_url or '127.0.0.1' in self.callback_url:
            logger.warning("⚠️ Using development URL. Ensure it's accessible from Azure Communication Services.")
    ...
    def suggest_fixes(self):
        ...
        if not self.acs_connection_string:
            ...
            logger.info("   az communication list-key --name <acs-resource-name> --resource-group <rg-name>")
            logger.info("   Then set: ACS_CONNECTION_STRING=endpoint=https://...;accesskey=...")
        
        # 2. Managed Identity Issues
        if not self.acs_connection_string and self.acs_endpoint:
            logger.info("\n2. Managed Identity Configuration:")
            logger.info("   Ensure your application has a managed identity and proper permissions:")
        ...
        logger.info("   Check with: az communication phonenumber list --connection-string '<connection-string>'")
    ...
    def create_acs_test_call_config(self):
        ...
        test_config = {
            ...
            "callback_url": f"{self.callback_url}/api/acs-callback" if self.callback_url else None,
            "websocket_url": f"{self.callback_url.replace('https://', 'wss://').replace('http://', 'ws://')}/ws/transcription" if self.callback_url else None,
            "acs_connection_string": "***hidden***" if self.acs_connection_string else None,
            "acs_endpoint": f"https://{self.acs_endpoint}" if self.acs_endpoint and not self.acs_endpoint.startswith('https://') else self.acs_endpoint,
        }
