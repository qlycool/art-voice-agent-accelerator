...
class APIEndpointsUser(FastHttpUser):
        ...
        with self.client.get("/health", catch_response=True) as response:
            ...
            if response.status_code == 200:
                self.performance_tracker.record_metric(
                    "health_check", duration_ms, True,
                    status_code=response.status_code
                )
            else:
                response.failure(f"Health check failed: {response.status_code}")
                self.performance_tracker.record_metric(
                    "health_check", duration_ms, False,
                    status_code=response.status_code
                )
    ...
    def inbound_call(self):
        ...
        with self.client.post(
            ...
            if response.status_code in [200, 201, 202]:
                self.performance_tracker.record_metric(
                    "inbound_call", duration_ms, True,
                    call_id=inbound_data["call_id"]
                )
            else:
                response.failure(f"Inbound call failed: {response.status_code}")
                self.performance_tracker.record_metric(
                    "inbound_call", duration_ms, False,
                    status_code=response.status_code
                )
    ...
    def acs_callback(self):
        ...
        callback_data = {
            ...
            "data": {
                "callConnectionId": self.call_id or f"call-{random.randint(1000, 9999)}",
    ...
    def get_call_metrics(self):
        ...
        with self.client.get(
            f"/api/call/{self.session_id}/metrics",
            catch_response=True
        ) as response:
            duration_ms = (time.time() - start_time) * 1000
