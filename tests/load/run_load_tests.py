...
from config import config, PerformanceTracker
...
class LoadTestOrchestrator:
    ...
    def _execute_single_test(self, test, test_config):
        ...
        cmd = [
            ...
            "-f", test_file,
            "--host", test_config.get('host', config.api_base_url),
            "--users", str(test['users']),
            "--spawn-rate", str(test['spawn_rate']),
            "--run-time", test['duration'],
            "--headless",
            "--csv", f"results/{test['file'].replace('.py', '')}",
            "--html", f"results/{test['file'].replace('.py', '')}_report.html"
        ]
    ...
    def _validate_performance_targets(self):
        ...
        targets = {
            "WebSocket handshake": {"target": f"< {config.websocket_handshake_target_ms}ms", "met": None},
            "End-to-end voice loop": {"target": f"< {config.end_to_end_voice_loop_target_ms}ms", "met": None},
            "STT processing": {"target": f"< {config.stt_final_segment_target_ms}ms", "met": None},
            "LLM TTFT": {"target": f"< {config.llm_ttft_target_ms}ms", "met": None},
            "TTS first byte": {"target": f"< {config.tts_first_byte_target_ms}ms", "met": None}
        }
        
        # This would be populated by parsing test outputs
        # For now, showing the framework
        for target_name, target_info in targets.items():
            status = "⏳ Unknown" if target_info["met"] is None else ("✅ Met" if target_info["met"] else "❌ Missed")
    ...
    def _save_json_report(self, test_config, report_time):
        ...
        report_filename = f"results/load_test_report_{report_time.strftime('%Y%m%d_%H%M%S')}.json"
...
def main():
    ...
    parser = argparse.ArgumentParser(description="RTMedAgent Comprehensive Load Testing")
    
    # Test configuration arguments
    parser.add_argument('--host', default=config.api_base_url, help='API host URL')
    parser.add_argument('--ws-host', default=config.websocket_base_url, help='WebSocket host URL')
    parser.add_argument('--api-users', type=int, default=25, help='Number of API test users')
    parser.add_argument('--service-users', type=int, default=20, help='Number of backend service test users')
    parser.add_argument('--websocket-users', type=int, default=15, help='Number of WebSocket test users')
    parser.add_argument('--integration-users', type=int, default=10, help='Number of integration test users')
    parser.add_argument('--parallel', action='store_true', help='Run tests in parallel where possible')
    parser.add_argument('--fail-fast', action='store_true', help='Stop on first critical test failure')
    parser.add_argument('--redis-url', default=config.redis_url, help='Redis connection URL')
    
    # Test selection arguments
    parser.add_argument('--test-suite', choices=['quick', 'standard', 'comprehensive'], 
                       default='standard', help='Test suite intensity')
