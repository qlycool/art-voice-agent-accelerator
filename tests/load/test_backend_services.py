...
class BackendServicesUser(User):
    ...
    def test_redis_session_operations(self):
        ...
        try:
            ...
            conversation = self.redis_client.hgetall(f"session:{self.session_id}:conversation")
            ...
            self.performance_tracker.record_metric("redis_update", update_duration, True)
    ...
    def test_openai_completion(self):
        ...
        try:
            ...
            prompt_messages = [
                ...
                {
                    "role": "user",
                    "content": TestDataGenerator.generate_medical_query()
                }
            ]
            
            # Simulate API call (replace with actual OpenAI call in real test)
            future = self.executor.submit(self._simulate_openai_request, prompt_messages)
            ...
            if result["success"]:
                ...
                self.performance_tracker.record_metric(
                    "openai_completion", duration, True,
    ...
    def test_cosmos_db_operations(self):
        ...
        try:
            ...
            # Record metrics
            self.performance_tracker.record_metric("cosmos_insert", insert_duration, True)
            self.performance_tracker.record_metric("cosmos_query", query_duration, True)
            self.performance_tracker.record_metric("cosmos_update", update_duration, True)
    ...
    def test_speech_services(self):
        ...
        try:
            ...
            stt_target_met = stt_result["duration_ms"] <= config.stt_final_segment_target_ms
            tts_target_met = tts_result["duration_ms"] <= config.tts_first_byte_target_ms
            
            self.performance_tracker.record_metric(
                "speech_stt", stt_result["duration_ms"], stt_result["success"],
                target_met=stt_target_met
            )
            self.performance_tracker.record_metric(
                "speech_tts", tts_result["duration_ms"], tts_result["success"],
            ...
            events.request.fire(
                ...
                response_time=total_duration,
                response_length=len(stt_result.get("transcript", "")) + tts_result.get("audio_bytes", 0),
                exception=None,
                context=self.context()
            )
...
def on_test_stop(environment, **kwargs):
    ...
    for service_type, service_list in service_stats.items():
        ...
        for name, stat in service_list:
            ...
            elif "openai" in name.lower() and stat.avg_response_time <= config.llm_ttft_target_ms:
                print(f"    ✓ OpenAI TTFT target met")
            elif "speech" in name.lower() and stat.avg_response_time <= config.stt_final_segment_target_ms:
