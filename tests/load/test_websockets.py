        ...
        try:
            ...
            if result["success"]:
                self.performance_tracker.record_metric(
                    "websocket_realtime", duration_ms, True,
            ...
            async with websockets.connect(ws_url) as websocket:
                ...
                for conversation_round in range(3):
                    ...
                    if stt_duration > config.stt_final_segment_target_ms:
                        print(f"STT latency exceeded target: {stt_duration}ms > {config.stt_final_segment_target_ms}ms")
                    ...
                    if agent_duration > config.end_to_end_voice_loop_target_ms:
                        print(f"Agent response exceeded target: {agent_duration}ms > {config.end_to_end_voice_loop_target_ms}ms")
        ...
        try:
            async with websockets.connect(ws_url) as websocket:
                ...
                for stream_cycle in range(5):  # 5 cycles of audio exchange
                    ...
                    try:
                        audio_response = await asyncio.wait_for(websocket.recv(), timeout=8.0)
                        response_data = json.loads(audio_response)
                        messages_received += 1
                        total_bytes += len(audio_response)
                        
                        if response_data.get("type") == "audio_data" and response_data.get("direction") == "outgoing":
                            # Successful audio processing
                            pass
                        else:
                            print(f"Unexpected audio response: {response_data.get('type')}")
...
def on_test_stop(environment, **kwargs):
    ...
    ws_stats = {name: stat for name, stat in stats.entries.items() if stat.method == "WebSocket"}
    ...
    for name, stat in ws_stats.items():
        ...
        if "realtime" in name and stat.avg_response_time <= config.end_to_end_voice_loop_target_ms:
...
if __name__ == "__main__":
    ...
    cmd = [
        ...
        "-f", __file__,
        "--host", config.websocket_base_url,
        "--users", "25",
        "--spawn-rate", "2",
        "--run-time", "3m",
        "--headless"
