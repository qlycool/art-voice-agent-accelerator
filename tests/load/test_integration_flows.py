...
from config import config, TestDataGenerator, PerformanceTracker
...
class IntegratedCallFlowUser(HttpUser):
    ...
    def complete_medical_consultation_flow(self):
        ...
        try:
            ...
            e2e_target_met = total_duration <= (config.end_to_end_voice_loop_target_ms * 10)  # Allow for multiple interactions
            
            success = (session_result["success"] and 
                      voice_result["success"] and 
                      completion_result["success"])
            
            self.performance_tracker.record_metric(
                "e2e_call_flow", total_duration, success,
                scenario=scenario,
    ...
    def _initiate_call(self, scenario):
        ...
        try:
            ...
            return {"success": False, "error": str(e), "duration_ms": (time.time() - start_time) * 1000}
    ...
    def _simulate_voice_interaction(self, session_id, scenario):
        ...
        pattern = conversation_patterns.get(scenario, conversation_patterns["routine_checkup_questions"])
        
        try:
            # Run voice interaction in separate thread to avoid blocking
            future = self.executor.submit(self._voice_interaction_worker, session_id, pattern)
            result = future.result(timeout=45)  # Allow up to 45 seconds for conversation
            return result
    ...
    def _voice_interaction_worker(self, session_id, pattern):
        ...
        try:
            ...
            return loop.run_until_complete(self._async_voice_interaction(session_id, pattern))
        except Exception as e:
        ...
        try:
            async with websockets.connect(ws_url) as websocket:
                ...
                init_message = {
                    ...
                    "audio_format": {"encoding": "pcm", "sample_rate": 16000, "channels": 1}
                }
                ...
                for i, question in enumerate(pattern["questions"]):
                    ...
                    try:
                        agent_response = await asyncio.wait_for(websocket.recv(), timeout=timeout)
                        ...
                        print(f"Timeout waiting for agent response to: {question[:30]}...")
                        break
    ...
    def _complete_call(self, session_id):
        ...
        try:
            ...
            completion_data = {
                ...
                "patient_satisfied": random.choice([True, True, True, False]),  # 75% satisfaction
    ...
    def test_concurrent_calls_capacity(self):
        ...
        try:
            ...
            for i in range(3):
                call_data = TestDataGenerator.generate_call_data()
                call_data["patient_id"] = f"{self.patient_profile['patient_id']}-concurrent-{i}"
                
                with self.client.post("/api/call", json=call_data, catch_response=True) as response:
                    if response.status_code in [200, 201]:
            ...
            for session_id in concurrent_sessions:
                if session_id:
                    with self.client.get(f"/api/call/{session_id}/status", catch_response=True) as response:
...
def on_test_stop(environment, **kwargs):
    ...
    integration_stats = {name: stat for name, stat in stats.entries.items() 
    ...
    if integration_stats:
        ...
        for name, stat in integration_stats.items():
            ...
            print(f"    95th percentile: {stat.get_response_time_percentile(0.95)/1000:.1f}s")
            
            # Success rate calculation
            success_rate = ((stat.num_requests - stat.num_failures) / stat.num_requests * 100) if stat.num_requests > 0 else 0
