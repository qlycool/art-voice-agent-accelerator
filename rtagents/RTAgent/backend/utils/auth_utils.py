...
def validate_azure_openai_auth_setup() -> Dict:
    ...
    try:
        if env_vars["AZURE_CLIENT_ID"]:
            ...
            try:
                credential = ManagedIdentityCredential(client_id=env_vars["AZURE_CLIENT_ID"])
    ...
    successful_tokens = [
        chain["token_info"] for chain in validation_results["credential_chain"] 
...
def validate_token_for_apim(credential) -> Dict:
    ...
    try:
        ...
        if missing_claims:
            validation["apim_compatibility"].append(f"Missing claims: {', '.join(missing_claims)}")
...
async def debug_jwt_token() -> Dict:
    ...
    try:
        ...
        if successful_credential:
            debug_info["validation_results"] = validate_token_for_apim(successful_credential)
        else:
