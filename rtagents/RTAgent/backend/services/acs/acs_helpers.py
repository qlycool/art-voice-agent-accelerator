...
async def play_response_with_queue(
    ...
    await _play_response_direct(
        ws, response_text, use_ssml, voice_name, locale, 
        participants, max_retries, initial_backoff, transcription_resume_delay
    )
...
async def process_message_queue(ws: WebSocket):
    ...
    try:
        ...
        logger.error(f"❌ Error processing message queue for call {call_connection_id}: {e}", exc_info=True)
    finally:
