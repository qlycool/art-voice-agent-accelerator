...
class TranscriptionHandler:
    ...
    def __init__(
        ...
        self.cm = cm
        self.redis_mgr = websocket.app.state.redis  # Assuming redis manager is stored in app state
        self.call_conn = websocket.app.state.call_conn  # Assuming call connection is stored in app state
        self.clients = websocket.app.state.clients  # Assuming clients are stored in app state
    ...
    async def handle_transcription_message(self, message: Dict[str, Any]) -> None:
        ...
        try:
            ...
            logger.debug(f"🔍 Raw transcription message: {message_json} | Session: {self.cm.session_id}")
            if message_json.get("kind") != "TranscriptionData":
                return

            bot_speaking = await self.cm.get_live_context_value(self.redis_mgr, "bot_speaking")
            ...
            logger.info(
                ...
                text, status, bot_speaking, self.cm.session_id
            )
    ...
    async def _handle_intermediate_transcription(self, text: str, bot_speaking: bool) -> None:
        ...
        logger.info(f"📊 Interrupt count updated: {interrupt_cnt + 1} | Session: {self.cm.session_id}")
    
    async def _handle_final_transcription(self, text: str) -> None:
        """Handle final transcription (user finished speaking)"""
        logger.info(f"📋 Final transcription received: '{text}' | Session: {self.cm.session_id}")
        ...
        logger.info(f"✅ Transcription processed and routed | Session: {self.cm.session_id}")
