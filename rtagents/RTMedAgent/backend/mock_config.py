...
def load_mock_config() -> MockConfig:
    ...
    return MockConfig(
        ...
        # Feature flags
        enable_realistic_responses=os.getenv("MOCK_REALISTIC_RESPONSES", "true").lower() == "true",
        enable_latency_simulation=os.getenv("MOCK_LATENCY_SIMULATION", "true").lower() == "true",
        enable_error_injection=os.getenv("MOCK_ERROR_INJECTION", "true").lower() == "true",
        
        # Performance settings
        track_performance_metrics=os.getenv("MOCK_TRACK_PERFORMANCE", "true").lower() == "true",
        log_slow_operations=os.getenv("MOCK_LOG_SLOW_OPS", "true").lower() == "true",
        slow_operation_threshold_ms=float(os.getenv("MOCK_SLOW_THRESHOLD_MS", "1000.0"))
    )

# Helper functions for mock services
async def simulate_latency(latency_range: tuple, operation_name: str = "operation") -> float:
...
class MockResponseGenerator:
    ...
    def generate_cosmos_document() -> dict:
        ...
        return {
            ...
            "intent": random.choice(["medication", "billing", "appointment", "general"]),
    ...
    def generate_redis_session() -> dict:
        ...
        return {
            ...
            "conversation_state": random.choice(["greeting", "collecting_info", "processing", "summary"]),
