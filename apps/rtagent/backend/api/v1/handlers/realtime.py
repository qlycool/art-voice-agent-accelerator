...
class V1RealtimeHandler:
            ...
            try:
                ...
                try:
                    if (
                        ...
                        and websocket.client_state.name not in ("DISCONNECTED", "CLOSED")
                    ):
    ...
    async def handle_browser_conversation(
        ...
        orchestrator_name = getattr(active_orchestrator, 'name', 'unknown') if active_orchestrator else 'legacy'

        session_id = None
        cm = None

        with trace_acs_operation(
            tracer, logger, "browser_conversation", 
            ...
            try:
                ...
                op.log_info(f"Browser conversation session started: {session_id} with {orchestrator_name}")
                ...
                if active_orchestrator:
                    cm.update_context("orchestrator_type", type(active_orchestrator).__name__)
                    if hasattr(active_orchestrator, 'get_config'):
                        cm.update_context("orchestrator_config", active_orchestrator.get_config())
                ...
                def on_partial(txt: str, lang: str):
                    logger.info(f"🗣️ User (partial) in {lang}: {txt} [session: {session_id}]")
                    ...
                    if websocket.state.is_synthesizing:
                        try:
                            ...
                            websocket.state.is_synthesizing = False
                            logger.info(f"🛑 TTS interrupted due to user speech [session: {session_id}]")
                        except Exception as e:
                ...
                def on_final(txt: str, lang: str):
                    logger.info(f"🧾 User (final) in {lang}: {txt} [session: {session_id}]")
                ...
                logger.info(f"STT recognizer started for V1 session {session_id} with orchestrator {orchestrator_name}")
                    ...
                    if msg.get("type") == "websocket.receive" and msg.get("bytes") is not None:
                        ...
                        if websocket.state.user_buffer.strip():
                            ...
                            if check_for_stopwords(prompt):
                                ...
                                # Send farewell audio
                                with trace_acs_dependency(
                                    tracer, logger, "tts_service", "send_farewell",
                                    session_id=session_id
                                ) as dep_op:
                            ...
                            with trace_acs_dependency(
                                tracer, logger, "conversation_orchestrator", "route_turn",
                                session_id=session_id, orchestrator_name=orchestrator_name
                            ) as dep_op:
                                dep_op.log_info(f"Routing user prompt: {prompt[:50]}...")
                ...
                op.log_info(f"Browser client disconnected normally: session {session_id}")
            except Exception as e:
                op.set_error(f"Browser conversation error: {e}")
            finally:
                # Cleanup
                with trace_acs_dependency(
                    tracer, logger, "cleanup_service", "session_cleanup",
                    session_id=session_id
                ) as cleanup_op:
                    # Stop TTS
                    try:
                    ...
                    try:
                        ...
                        logger.error(f"Error persisting analytics for session {session_id}: {e}")

                    cleanup_op.log_info(f"Browser conversation cleanup completed: session {session_id}")
